...
const DOCUMENT_EXTENSIONS = new Set(["jpeg", "jpg", "mp4", "pdf", "png"]);
const DOCUMENT_TYPES = new Set(["application/pdf", "image/jpeg", "image/png", "video/mp4"]);
const AVATAR_EXTENSIONS = new Set(["jpeg", "jpg", "png", "webp"]);
const AVATAR_TYPES = new Set(["image/jpeg", "image/png", "image/webp"]);

export const uploadPolicies = {
  document: { extensions: DOCUMENT_EXTENSIONS, types: DOCUMENT_TYPES, maxBytes: Number(process.env.FILE_UPLOAD_MAX_BYTES || 50 * 1024 * 1024), maxNameLength: 150 },
...
export function prepareUpload(req, policy, { fallbackName = "archivo" } = {}) {
  ...
  if (!policy.extensions.has(extension(name)) || !policy.types.has(contentType)) {
