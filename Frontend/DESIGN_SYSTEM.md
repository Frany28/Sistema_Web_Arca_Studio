...
- El selector de país de `Input` con tipo `Phone number` utiliza todo el contenedor de bandera, prefijo y flecha como una zona interactiva. El signo `+` es fijo y no editable; únicamente los dígitos del prefijo funcionan como `combobox`: escribirlos filtra por código telefónico y una coincidencia exacta selecciona el país. Pulsar la bandera, el espacio del contenedor o la flecha abre las opciones. El menú se une visualmente al borde inferior sin separación, hereda exactamente el ancho del selector, muestra como máximo cuatro países de `35px` y habilita scrollbar vertical para el resto. Cada opción alinea bandera, prefijo y abreviatura compacta; el nombre completo se conserva como información accesible. Mantiene selección visible, cierre con `Escape` y tokens de `DropdownMenu`.
