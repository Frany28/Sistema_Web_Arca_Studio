...
- El segundo contenedor de Proyectos destacados es la galería de Quinta Bella Vista (`4686:3913`): seis imágenes originales, tres columnas de alturas alternadas, separación de 24 px y padding vertical de 48 px/horizontal de 24 px. El siguiente gesto tras completar el encabezado desplaza hasta la galería y la revela con la máscara compartida. Se espera su animación antes de aceptar otro paso; al subir, si ambos contenedores ya terminaron de aparecer, se regresa directamente a Servicios. El navbar mantiene seleccionado Proyectos destacados durante ambos contenedores.

- El avance por scroll desde Servicios exige haber mostrado todas las categorías. Una categoría solo cuenta como visitada al terminar su transición; una selección interrumpida no completa el recorrido. Mientras una transición esté pendiente, el controlador mantiene bloqueada la salida descendente. Los saltos explícitos del navbar conservan su excepción.

- Proyectos destacados sigue a Servicios dentro de Home (`#featured-projects`). Su primer contenedor corresponde a Figma `4856:5032`: encabezado, nombre Quinta Bella Vista y descripción, con padding de 48 px, separación de 24 px y descripción limitada a 520 px. Entra vacío y el siguiente gesto revela el contenedor completo con la misma máscara y transición compartidas de Servicios (getSectionRevealClip y getSectionRevealTransition); movimiento reducido muestra el texto inmediatamente. No se puede salir por scroll hasta completar el revelado. El navbar indica la sección activa y permite saltar directamente, incluso durante otra transición. Servicios debe completar sus categorías antes de pasar por scroll a Proyectos destacados; regresar reinicia los contenedores de Servicios ocultos. Los demás contenedores de proyectos siguen pendientes de diseño.
...
- La navegación por rueda, touch, teclado y scrollbar no permite abandonar la sección actual mientras tenga pasos de scroll pendientes. Seleccionar un área en el navbar es una excepción: desplaza directamente al destino y puede interrumpir una transición en curso. En Servicios, una vez revelados ambos contenedores, subir regresa directamente al video sin ocultaciones individuales; el scrollbar no puede omitir la introducción ni dar por completado el efecto del video.
...
- El selector consume internamente los gestos de rueda y touch para cambiar categoría, imagen e indicador. En la última categoría, tras visitar todas, el siguiente gesto descendente solicita al controlador de Home la transición a Proyectos destacados; la inercia del gesto anterior no debe activar esa salida. No utiliza el scroll de Home, pin ni espaciadores para recorrer categorías. No se añaden botones «Volver al inicio» o «Pausar video»: la salida directa se mantiene en el navbar. El video se pausa fuera de su sección, con la pestaña oculta y cuando se solicita movimiento reducido.
- La rueda con Ctrl conserva el zoom del navegador. El navbar reaparece al recibir foco de teclado y permanece visible mientras el foco siga dentro. Las pruebas unitarias del scroll cubren temporizadores, inercia, variantes de rueda, teclado, scrollbar, navegación directa, bloqueo de revelados y categorías; no sustituyen una validación visual en navegador.
