...
- El selector de país de `Input` con tipo `Phone number` utiliza todo el contenedor de bandera, prefijo y flecha como un único botón; el prefijo no se edita manualmente y cambia al seleccionar una opción. El menú se une visualmente al borde inferior sin separación, hereda exactamente el ancho del botón, muestra como máximo cuatro países de `35px` y habilita scrollbar vertical para el resto. Cada opción alinea bandera, prefijo y abreviatura compacta; el nombre completo se conserva como información accesible. Mantiene selección visible, cierre con `Escape` y tokens de `DropdownMenu`.
