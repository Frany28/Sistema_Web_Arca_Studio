...
- El selector de país de `Input` con tipo `Phone number` utiliza todo el contenedor de bandera, prefijo y flecha como una zona interactiva. El signo `+` es fijo y no editable; únicamente los dígitos del prefijo funcionan como `combobox`: escribirlos filtra por código telefónico y una coincidencia exacta selecciona el país. Pulsar la bandera, el espacio del contenedor o la flecha abre las opciones; la flecha no utiliza tooltip en este contexto. El menú se une al borde inferior, alinea sus bordes y ancho exactamente con el selector, muestra como máximo cuatro países de `35px` y reutiliza el componente `ScrollBar`, ocultando el scrollbar nativo. Cada opción alinea bandera, prefijo y abreviatura compacta; el nombre completo se conserva como información accesible. Mantiene selección visible y cierre con `Escape`.
