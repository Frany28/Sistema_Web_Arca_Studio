...
export function render(device, frame) {
    ...
    if (s.zoomAspect !== aspect) {
        ...
        const requiredRadius = (cameraDistance *
            diagonal /
            Math.sqrt(targetFocalLength * targetFocalLength +
                diagonal * diagonal));
        ...
        const perspectiveDepth = Math.sqrt(Math.max(0.0001, cameraDistance * cameraDistance -
            conservativeRadius * conservativeRadius));
        const unclampedMinimumCoverZoom = (diagonal *
            perspectiveDepth /
            (baseFocalLength * conservativeRadius)) * 1.12;
...
defineProperties(Effect, {
    ...
    gradientMethod: {
        ...
        control: "select",
        options: [
            { value: 0, label: "Height" },
            { value: 1, label: "Noise" },
            { value: 2, label: "Facing" },
        ],
    },
});
