 ...
 const completeTitleReveal = useCallback((panelIndex) => {
  ...
  useLayoutEffect(() => {
    ...
    const runtime = {
      ...
      supportsScrollEnd: "onscrollend" in scroller,
    ...
    runtime.requestAnimationFrame = (callback) => {
      ...
      frameId = window.requestAnimationFrame(() => {
        runtime.animationFrames.delete(frameId);
        callback();
      });
    ...
    const statement = createHomeStatementController({
      ...
      isPanelTransitioning: () => Boolean(runtime.activeTween),
    ...
    return () => {
      ...
      runtime.animationFrames.forEach((frameId) => {
        window.cancelAnimationFrame(frameId);
      });
