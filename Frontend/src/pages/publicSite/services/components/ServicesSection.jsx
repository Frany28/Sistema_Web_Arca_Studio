...
function ServicesSection({ step = 0, onRevealComplete, onCategoriesComplete }) {
  return (
    <section id="services" aria-label="Servicios" className="dark flex min-h-dvh flex-col gap-[var(--spacing-gap-8)] bg-[var(--color-neutral-950-uniform)] pt-[var(--spacing-gap-9)] pb-[var(--spacing-gap-8)]">
      ...
      <ServicesCategoryShowcase key={step >= 2 ? "visible" : "hidden"} categories={SERVICES_CATEGORIES} visible={step >= 2} onRevealComplete={onRevealComplete} onCategoriesComplete={onCategoriesComplete} />
    </section>
  );
}
